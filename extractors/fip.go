...
func (extractor FipExtractor) Playlist(timestampFrom int64) ([]playlist.Track, error) {
	// FIP uses graphql to serve its tracks history. To get the history for any given date and time, issue a GET to www.fip.fr/latest/api/graphql with a query string containing the following:
	// operationName=History
	// variables={"first": <int>, "after": <base64 encoded seconds epoch>, "stationgId": <int>}
	// extensions={"persistedQuery: {"version": 1, "sha256Hash": "f8f404573583a6a9410cd24637f214a0b93038696c1d20f19202111b51fd8270"}
	// It returns an extensive object containing `first` number of tracks played since `after` timestamp
	first := 10
	from := time.Now().Unix()
